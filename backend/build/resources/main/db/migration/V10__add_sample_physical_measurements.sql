...
sample_mass AS (
    ...
    SELECT 'sampleMass', 'Sample Mass', 'Mass of the sample.',
           'QUANTITY', NULL, '["quecto(qg)", "ronto(rg)", "yocto(yg)","zepto(zg)", "atto(ag)", "femto(fg)","pico(pg)", "nano(ng)", "micro(μg)", "milli(mg)", "gram(g)", "kilo(kg)", "mega(Mg)", "giga(Gg)"]'::jsonb, NULL, sd.id, false, 80, 'https://matwerk.datamanager.kit.edu/skosmos/SampleDescriptionVocabulary-1/de/page/SampleMass?clang='
    FROM sample_description sd
    RETURNING id
),
